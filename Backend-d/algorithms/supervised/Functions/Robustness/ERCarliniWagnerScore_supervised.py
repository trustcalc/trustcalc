def get_er_carlini_wagner_score_supervised(model=not None, training_dataset= None, test_dataset=not None, factsheet= None, mappings=not None,target_column=None, outliers_data=None, thresholds=None, outlier_thresholds=None, outlier_percentage=None, high_cor= None,print_details=True):
    ...
    if not thresholds:
        thresholds = mappings["robustness"]["score_carlini_wagner_attack"]["thresholds"]["value"]
...
print(get_er_carlini_wagner_score_supervised(test_dataset=test, model=model,mappings=mappings))"""